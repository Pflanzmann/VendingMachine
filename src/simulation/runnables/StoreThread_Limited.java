...
public class StoreThread_Limited implements Runnable {
    ...
    public StoreThread_Limited(String threadName, PrintStream writer, VendingMachine[] vendingMachines, String MANUFACTURER_NAME, Object storeLock) {
        ...
        while (true) {
            synchronized (storeLock) {
                ...
                for (VendingMachine vendingMachine : vendingMachines) {
                    cakes = vendingMachine.getAllCakes();
                    if (ArrayHelper.getArrayCount(cakes) != cakes.length) {
                        notAllFull = true;
                    }
                }
